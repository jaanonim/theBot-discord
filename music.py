...
# Donwload dose not effect playlist
download = False
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def create_playlist(cls, ctx, loop):
...
class Music(commands.Cog):
    ...
    @commands.command(name="pp", description="streams music")
    async def play_playlist(self, ctx):
        ...
        source = await YTDLSource.create_playlist(ctx, loop=self.bot.loop)
        ...
        embed = discord.Embed(
            title="Queue",
            description="Added playlist to queue.",
            color=discord.Color.green(),
        )
        await ctx.send(embed=embed)
